...
<div class="flex flex-row justify-center items-center text-[24px] font-lido gap-40 ml-20">
	<div class="flex flex-col gap-5 items-center justify-center text-center">
		<img
			src={`/characters/${currentCharacter?.name}/Attack/attack2.png`}
			alt="Character Image"
			class="w-[350px] h-[350px]"
		/>
		<div class="flex flex-row gap-5 items-center justify-center">
			<button
				on:click={() => changeCharacter(-1)}
				class="hover:brightness-75 ease-in-out duration-300"
			>
	...
	<div class="flex flex-col items-start justify-center gap-2 p-5">
		...
		<div class="text-[16px]">Current lobby: {currentUser?.lobby?.name ?? 'None'}</div>
